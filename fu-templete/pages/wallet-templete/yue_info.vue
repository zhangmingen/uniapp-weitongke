<template>
  <view class="page">
    <view class="info_list" v-for="(item,index) in listData" :key="index">
      ...
      <text class="info_pc" :class="item.change_type==1?'modes':'jianfu'">{{item.change_type==1?'+'+item.change_money:item.change_money}}</text>
    </view>
    <view class="zanwu" :class="hidden?'act':''">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        hidden: false,
        minixPagingListsApi: global.apiUrls.getMoneyDetail,
