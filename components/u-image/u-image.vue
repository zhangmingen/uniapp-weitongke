<template>
	<view class="u-image" @tap="onClick" :style="[wrapStyle, backgroundStyle]">
		...
		<view v-if="showLoading && loading" class="u-image__loading" :style="{
			...
			<view v-else :name="loadingIcon" :width="width" :height="height" class="cuIcon-loading lg rotate"></view>
		</view>
		<view v-if="showError && isError && !loading" class="u-image__error" :style="{
				borderRadius: shape == 'circle' ? '50%' : addUnit(borderRadius)
			}">
			<slot v-if="$slots.error" name="error" />
			<view v-else :name="errorIcon" :width="width" :height="height" class="cuIcon-warn lg"></view>
		</view>
	</view>
</template>
