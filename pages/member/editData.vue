		...
		<form @submit="formSubmit">
			<view class="padding-sm edit-box">
				...
				<view class="flex solid-bottom item">
					...
					<picker :end="yearEnd" @change="bindDateChange" :value="dateValue" mode="date" fields="year" name="birth_year" class="flex-sub text-lg text-right">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.name = userInfo.address ? userInfo.address : '请选择省份';
		...
		methods: {
			...
			formSubmit: function(e) {
				...
				timer = setTimeout(() => {
					...
					// 简介图片
					formData.introduction_img = this.imgList_id.length>0 ? this.imgList_id.join(',') : '';
