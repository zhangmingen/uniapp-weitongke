<template>
	<view :class="classType==1?'pt212':'pt110'">
		<view class="fixed-top bg-white">
			<view class="cu-bar bg-white search">
				<view class="search-form round margin-0">
					<text class="cuIcon-search text-black"></text>
					<input type="text" class="text-df" placeholder="请输入关键词" confirm-type="search" @tap="handleJump" :data-url="'/pages/class/search?type='+classType"></input>
				</view>
			</view>
			<!-- nav -->
			<view class="class_nav" v-if="navArr.length>0 && classType==1">
			...
			<view>
				...
				<!-- 展开分类 -->
				<view class="text-df" v-else :class="navArr.length>0?'unfold_box':''">
		...
		<!-- 遮罩层 -->
		<view class="mask" v-if="!unfoldStatus" @tap="unfoldFun" @touchmove.stop.prevent></view>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				unfoldStatus: true,
				// 配置分页接口地址
				minixPagingListsApi: global.apiUrls.PostClassList,
				// 默认进去不请求
		...
		onLoad(options) {
			...
			var optionsType = options.type || 1;
			uni.setNavigationBarTitle({
				title: optionsType == 3 ? '同课·关注' : optionsType == 2 ? '同课·推荐' : '同课·发现'
			})
